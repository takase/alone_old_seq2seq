...
from fairseq.modules import (
    ...
    SinusoidalPositionalEmbedding, OneEmbed
)
...
class TransformerModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        #add for one embedding
        parser.add_argument('--one-emb', default=None, choices=['binary', 'real'],
                            help='use one embedding for all tokens')
        parser.add_argument('--one-emb-dropout', type=float, default=0.5,
                            help='dropout ratio for one embedding for binary mask')
        parser.add_argument('--one-emb-std', type=float, default=1.0,
                            help='standard deviation for one embedding real number filter')
        parser.add_argument('--codenum', type=int, default=64,
                            help='the number of codes in one codebook')
        parser.add_argument('--codebooknum', type=int, default=8,
                            help='the number of codebooks')
        parser.add_argument('--one-emb-inter-dim', default=0, type=int,
                            help='the vector size of internal dimension')
        parser.add_argument('--one-emb-layernum', default=2, type=int,
                            help='the number of layers for embedding construction')
        parser.add_argument('--one-emb-relu-dropout', default=0.1, type=float,
                            help='rate of dropout to activated (relu) vector in embedding construction')
        parser.add_argument('--stop-relu-dropout-update', default=0, type=int,
                            help='update num to stop relu dropout')
    ...
    def build_model(cls, args, task):
        ...
        mask_file_prefix = args.save_dir + '/mask_to_one_embed'
        if args.share_all_embeddings:
            ...
            encoder_embed_tokens = build_embedding(
                src_dict, args.encoder_embed_dim, args, mask_file_prefix + '_encoder.pt', args.encoder_embed_path
            )
            ...
            decoder_embed_tokens = build_embedding(
                tgt_dict, args.decoder_embed_dim, args, mask_file_prefix + '_decoder.pt', args.decoder_embed_path
            )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, target_length, encoder_out=None, incremental_state=None):
        """
        Args:
        ...
        if self.adaptive_softmax is None:
            ...
            if self.share_input_output_embed:
                if self.embed_tokens.weight.size(0) == 1:
                    #in the case using one embed
                    output_matrix = self.embed_tokens.construct_matrix_for_output_layer()
                    x = F.linear(x, output_matrix)
                else:
                    #ordinary embedding
    ...
    def upgrade_state_dict(self, state_dict):
        ...
        if isinstance(self.embed_positions_original, SinusoidalPositionalEmbedding):
            if 'decoder.embed_positions_original.weights' in state_dict:
                del state_dict['decoder.embed_positions_original.weights']
            state_dict['decoder.embed_positions_original._float_tensor'] = torch.FloatTensor(1)
        if isinstance(self.embed_positions_lrpe, SinusoidalPositionalEmbedding):
            if 'decoder.embed_positions_lrpe.weights' in state_dict:
                del state_dict['decoder.embed_positions_lrpe.weights']
            state_dict['decoder.embed_positions_lrpe._float_tensor'] = torch.FloatTensor(1)
        if isinstance(self.embed_positions_ldpe, SinusoidalPositionalEmbedding):
            if 'decoder.embed_positions_ldpe.weights' in state_dict:
                del state_dict['decoder.embed_positions_ldpe.weights']
            state_dict['decoder.embed_positions_ldpe._float_tensor'] = torch.FloatTensor(1)
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state,
                ...
                self_attn_padding_mask=None, pos=None, len4emb=None):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
        ...
        if pos is None:
            ...
            residual = x + pos
        if len4emb is not None:
            residual = len4emb.view(-1, 1).unsqueeze(0).expand_as(residual).float() * residual
...
def OneEmbedding(num_embeddings, embedding_dim, padding_idx, one_emb_type, dropout, std, codenum, codebooknum, layernum, interdim, relu_dropout, mask_file):
    m = OneEmbed(num_embeddings, embedding_dim, padding_idx, one_emb_type, dropout, std, codenum, codebooknum, layernum, interdim, relu_dropout, mask_file)
    if one_emb_type == 'binary':
        nn.init.normal_(m.weight, mean=0, std=(embedding_dim+interdim) ** -0.05)
    else:
        nn.init.normal_(m.weight, mean=0, std=(embedding_dim+interdim) ** -0.3)
